...
:deal0
    ...
    %dd%DOSBox -noconsole^
    -c "mount c \"%aa%\"" -c "c:"^
    -c "tasm/t/zi T.ASM>T.txt" -c "tlink/v/3 T.OBJ>>T.txt"-c "T.EXE>>T.txt"^
    -c "EXIT"
    echo.>>T.txt
    type T.txt
    exit
...
:deal3
    ...
    copy TDCONFIG1 TDCONFIG.TD REM 使得td.exe 使用这个配置文件
    %dd%DOSBox -conf %dd%bigbox.conf -noautoexec -c "mount c \"%aa%\"" -c "c:"^
    -c "tasm/zi T.ASM" -c "tlink/v/3 T.OBJ"-c "Td t"
    exit
...
:deal5
    ...
    exit
:deal6
    echo add tasm to path and dosbox here
    %dd%DOSBox -conf %dd%bigbox.conf -noautoexec^
