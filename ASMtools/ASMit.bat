...
copy %3 T.ASM>nul
::根据%1指令选择不同的编译运行操作
if %1==0 goto deal0 REM 将文件复制到Test文件夹中，dosbox打开该文件夹
if %1==1 goto deal1 REM 在终端中输出结果，code runner没有设置在终端中运行的话会在输出中输出结果
if %1==2 goto deal2 REM 在dosbox中汇编链接运行
if %1==3 goto deal3 REM 在dosbox中运行程序，按任意键结束关闭dosbox，对于一些会检测键盘状态的程序，比如chapter 4/OC.asm会自动关闭
if %1==4 goto deal4 REM 在dosbox中汇编链接调试
if %1==5 goto deal5 REM 在终端中输出结果，code runner没有设置在终端中运行的话会在输出中输出结果
if %1==6 goto deal6 REM 在dosbox中汇编链接运行
if %1==7 goto deal7 REM 在dosbox中运行程序，按任意键结束关闭dosbox，对于一些会检测键盘状态的程序，比如chapter 4/OC.asm会自动关闭
if %1==8 goto deal8 REM 在dosbox中汇编链接调试
